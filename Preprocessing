...
def noiseRemover(df):
    ...
    '''External or Internal distracted driving behavior classification'''
    list_distraction = ['A', 'B', 'C', 'D']
    list_condition = ['B', 'C']
    df.loc[df['DR_DISTRACT_CD'].isin(list_distraction), 'Distraction_Label'] = 'External'
    df.loc[(df['DR_DISTRACT_CD'].isin(list_distraction) == False) & (
        df['DR_COND_CD'].isin(list_condition)), 'Distraction_Label'] = 'Internal'
    df.loc[
        (df['Distraction_Label'] != 'Internal') & (df['Distraction_Label'] != 'External'), 'Distraction_Label'] = 'None'
    ...
    for i in tqdm(list_distract):
        ...
        sleep(0.1)
    df.loc[(df['DR_DISTRACT_CD'].isin(list_distract) == False) & (
        df['DR_COND_CD'].isin(list_condition)), 'drive_distract'] = 'E'
...
df_cleaned.to_csv('10YearsData.csv', index = False)
